...
common_descriptions = {
  ...
  'VHS_PATH': [
    ...
    video('https://github.com/Kosinkadink/KKvideo/assets/4284322/729b7185-1fca-41d8-bc8d-a770bb2a5ce6'),
    ...
    "GetCount": ['Get {cap} Count KKvideo', short_desc('Return the number of {lowers} in an input as an INT'),
    {'Inputs': {
    ...
    "SelectEveryNth": ['Select Every Nth {cap} KKvideo', short_desc('Keep only 1 {lower} for every interval'),
    {'Inputs': {
        '{lowers}': 'The input {lower}',
        },
     'Outputs': {
...
descriptions = {
  'KKvideo_VideoCombine': ['Video Combine KKvideo', short_desc('Combine an image sequence into a video'), {
    'Inputs': {
  ...
  'KKvideo_LoadVideo': ['Load Video KKvideo', short_desc('Loads a video from the input folder'),
    {'Inputs': {
  ...
  'KKvideo_LoadVideoFFmpeg': ['Load Video FFmpeg KKvideo', short_desc('Loads a video from the input folder using ffmpeg instead of opencv'),
  ...
  'KKvideo_LoadVideoPath': ['Load Video (Path) KKvideo', short_desc('Loads a video from an arbitrary path'),
    {'Inputs': {
        'meta_batch': '(optional) Connect to a Meta Batch manager to divide extremely long sequences into sub batches. See the documentation for Meta Batch Manager',
        'vae': ['(optional) If provided the node will output latents instead of images. This drastically reduces the required RAM (not VRAM) when working with long (100+ frames) sequences',
                'Using this is strongly encouraged unless connecting to a node that requires a blue image connection such as Apply Controllnet',
  ...
  'KKvideo_LoadVideoFFmpegPath': ['Load Video FFmpeg (Path) KKvideo', short_desc('Loads a video from an arbitrary path using ffmpeg instead of opencv'),
    'Provides faster execution speed, transparency support, and allows specifying start time in seconds',
    {'Inputs': {
        'meta_batch': '(optional) Connect to a Meta Batch manager to divide extremely long sequences into sub batches. See the documentation for Meta Batch Manager',
        'vae': ['(optional) If provided the node will output latents instead of images. This drastically reduces the required RAM (not VRAM) when working with long (100+ frames) sequences',
  ...
  'KKvideo_LoadImages': ['Load Images KKvideo', short_desc('Loads a sequence of images from a subdirectory of the input folder'),
    {'Inputs': {
        'meta_batch': '(optional) Connect to a Meta Batch manager to divide extremely long sequences into sub batches. See the documentation for Meta Batch Manager',
        },
     'Outputs': {
  ...
  'KKvideo_LoadImagesPath': ['Load Images (Path) KKvideo', short_desc('Loads a sequence of images from an arbitrary path'),
    {'Inputs': {
        'meta_batch': '(optional) Connect to a Meta Batch manager to divide extremely long sequences into sub batches. See the documentation for Meta Batch Manager',
        },
     'Outputs': {
  ...
  'KKvideo_LoadImagePath': ['Load Image (Path) KKvideo', short_desc('Load a single image from a given path'),
    {'Inputs': {
  ...
  "KKvideo_LoadAudio": ['Load Audio (Path) KKvideo', short_desc('Loads an audio file from an arbitrary path'),
    {'Outputs': {
  ...
  "KKvideo_LoadAudioUpload": ['Load Audio (Upload) KKvideo', short_desc('Loads an audio file from the input directory'),
  ...
  "KKvideo_AudioToVHSAudio": ['Audio to legacy VHS_AUDIO KKvideo', short_desc('utility function for compatibility with external nodes'),
  ...
  "KKvideo_VHSAudioToAudio": ['Legacy VHS_AUDIO to Audio KKvideo', short_desc('utility function for compatibility with external nodes'),
    "VHS used to use an internal VHS_AUDIO format for routing audio between inputs and outputs. This format was intended to only be used internally and was designed with a focus on performance over ease of use. Since ComfyUI now has an internal AUDIO format, VHS now uses this format. However, some custom node packs were made that are external to both ComfyUI and VHS that use VHS_AUDIO. This node was added so that those external nodes can still function",
    {'Inputs': {
  ...
  "KKvideo_PruneOutputs": ['Prune Outputs KKvideo', short_desc('Automates deletion of undesired outputs from a Video Combine node.'),
  ...
  "KKvideo_BatchManager": ['Meta Batch Manager KKvideo', short_desc('Split the processing of a very long video into sets of smaller Meta Batches'),
    ...
    image("https://github.com/Kosinkadink/KKvideo/assets/4284322/7cb3fb7e-59d8-4cb2-a09f-9c6698de8b1f"),
  ...
  "KKvideo_VideoInfo": ['Video Info KKvideo', short_desc('Splits information on a video into a numerous outputs'),
    {'Inputs': {
  ...
  "KKvideo_VideoInfoSource": ['Video Info Source KKvideo', short_desc('Splits information on a video into a numerous outputs describing the file itself without accounting for load options'),
    {'Inputs': {
        'video_info': 'A connection to a Load Video node',
        },
     'Outputs': {
  ...
  "KKvideo_VideoInfoLoaded": ['Video Info Loaded KKvideo', short_desc('Splits information on a video into a numerous outputs describing the file itself after accounting for load options'),
    {'Inputs': {
        'video_info': 'A connection to a Load Video node',
        },
     'Outputs': {
  ...
  "KKvideo_SelectFilename": ['VAE Select Filename KKvideo', short_desc('Select a single filename from the VHS_FILENAMES output by a Video Combine and return it as a string'),
    ...
    # Batched Nodes
  "KKvideo_VAEEncodeBatched": ['VAE Encode Batched KKvideo', short_desc('Encode images as latents with a manually specified batch size.'),
  ...
  "KKvideo_VAEDecodeBatched": ['VAE Decode Batched KKvideo', short_desc('Decode latents to images with a manually specified batch size'),
  ...
  "KKvideo_SplitLatents": ['Split Latents KKvideo', short_desc('Split a set of latents into two groups'),
    {'Inputs': {
    ...
    "KKvideo_SplitImages": ['Split Images KKvideo', short_desc('Split a set of images into two groups'),
    {'Inputs': {
    ...
    "KKvideo_SplitMasks": ['Split Masks KKvideo', short_desc('Split a set of masks into two groups'),
    {'Inputs': {
    ...
    "KKvideo_MergeLatents": ['Merge Latents KKvideo', short_desc('Combine two groups of latents into a single group of latents'),
    {'Inputs': {
    ...
    "KKvideo_MergeImages": ['Merge Images KKvideo', short_desc('Combine two groups of images into a single group of images'),
    {'Inputs': {
    ...
    "KKvideo_MergeMasks": ['Merge Masks KKvideo', short_desc('Combine two groups of masks into a single group of masks'),
    {'Inputs': {
        'mask_A': 'The first group of masks',
    ...
    "KKvideo_GetLatentCount": format_type(common_descriptions['GetCount'], 'latent'),
    "KKvideo_GetImageCount": format_type(common_descriptions['GetCount'], 'image'),
    "KKvideo_GetMaskCount": format_type(common_descriptions['GetCount'], 'mask'),
    "KKvideo_DuplicateLatents": ['Repeat Latents KKvideo', short_desc('Append copies of a latent to itself so it repeats'),
    {'Inputs': {
    ...
    "KKvideo_DuplicateImages": ['Repeat Images KKvideo', short_desc('Append copies of a image to itself so it repeats'),
    {'Inputs': {
    ...
    "KKvideo_DuplicateMasks": ['Repeat Masks KKvideo', short_desc('Append copies of a mask to itself so it repeats'),
    {'Inputs': {
    ...
    "KKvideo_SelectEveryNthLatent": format_type(common_descriptions['SelectEveryNth'], 'latent'),
    "KKvideo_SelectEveryNthImage": format_type(common_descriptions['SelectEveryNth'], 'image'),
    #TODO: fix discrepency of input being mask instead of masks?
    "KKvideo_SelectEveryNthMask": format_type(common_descriptions['SelectEveryNth'], 'mask', lowers='mask'),
    #TODO: port documentation for select nodes to new system
    #"KKvideo_SelectLatents": None,
    #"KKvideo_SelectImages": None,
    #"KKvideo_SelectMasks": None,
  "KKvideo_Unbatch": ['Unbatch KKvideo', short_desc('Unbatch a list of items into a single concatenated item'),
  ...
  "KKvideo_SelectLatest": ['Select Latest KKvideo', short_desc('Experimental virtual node to select the most recently modified file from a given folder'),
