...
def get_upload_video_input(default=""):
    return ("STRING", {
        "default": default,
        "placeholder": "Paste full video URL here",
    })

def resolve_upload_video(video):
    video = strip_path(video)
    if is_url(video):
        return try_download_video(video, folder_paths.get_input_directory()) or video
...
class LoadVideoUpload:
    ...
    def INPUT_TYPES(s):
        return {"required": {
                    "video_url": get_upload_video_input(),
                ...
                "optional": {
                    "meta_batch": ("KKvideo_BatchManager",),
                    "vae": ("VAE",),
    ...
    def load_video(self, **kwargs):
        kwargs['video'] = resolve_upload_video(kwargs['video'])
        return load_video(**kwargs)

    @classmethod
    def IS_CHANGED(s, video, **kwargs):
        if is_url(strip_path(video)):
            return hash_path(video)
        image_path = folder_paths.get_annotated_filepath(strip_path(video))
...
class LoadVideoFFmpegUpload:
    @staticmethod
    def get_video_url(kwargs):
        return kwargs.get("video_url", kwargs.get("video", ""))
        ...
        for f in os.listdir(input_dir):
            if os.path.isfile(os.path.join(input_dir, f)):
                ...
                if len(file_parts) > 1 and (file_parts[-1].lower() in video_extensions):
                    files.append(f)
        default_video = sorted(files)[0] if files else ""
        return {"required": {
                    "video": get_upload_video_input(default_video),
                    "force_rate": (floatOrInt, {"default": 0, "min": 0, "max": 60, "step": 1, "disable": 0}),
                    "custom_width": ("INT", {"default": 0, "min": 0, "max": DIMMAX, 'disable': 0}),
                    "custom_height": ("INT", {"default": 0, "min": 0, "max": DIMMAX, 'disable': 0}),
                    "frame_load_cap": ("INT", {"default": 0, "min": 0, "max": BIGMAX, "step": 1, "disable": 0}),
    ...
    def load_video(self, **kwargs):
        video_url = self.get_video_url(kwargs)
        kwargs['video'] = resolve_upload_video(video_url)
    ...
    def IS_CHANGED(s, **kwargs):
        video_url = s.get_video_url(kwargs)
        if not strip_path(video_url):
            return ""
        if is_url(strip_path(video_url)):
            return hash_path(video_url)
        image_path = folder_paths.get_annotated_filepath(strip_path(video_url))
        return calculate_file_hash(image_path)

    @classmethod
    def VALIDATE_INPUTS(s, **kwargs):
        video_url = s.get_video_url(kwargs)
        video_url = strip_path(video_url)
        if not video_url:
            return "Please paste a video URL."
        if is_url(video_url):
            return True
        if not folder_paths.exists_annotated_filepath(video_url):
            return "Invalid video file: {}".format(video_url)
        return True
