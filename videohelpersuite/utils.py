...
download_history = {}
def get_download_filename(url, content_type=None):
    ...
    if "." not in filename:
        extension = {
            ...
            "image/gif": ".gif",
        }.get((content_type or "").split(";")[0].lower(), ".mp4")
        filename += extension
    return filename

def get_available_download_path(download_dir, filename):
    base, ext = os.path.splitext(filename)
    path = os.path.join(download_dir, filename)
    index = 1
    while os.path.exists(path):
        path = os.path.join(download_dir, f"{base}_{index}{ext}")
        index += 1
    return path

def download_direct_video(url, download_dir):
    ...
    with urllib.request.urlopen(request, timeout=60) as response:
        filename = get_download_filename(url, response.headers.get("Content-Type"))
        file = get_available_download_path(download_dir, filename)
...
def try_download_video(url, download_dir=None):
    ...
    if download_dir is None:
        download_dir = folder_paths.get_temp_directory()
    ...
    if ytdl_path is not None:
        try:
            res = subprocess.run([ytdl_path, "--no-simulate",
                                  "--print", "after_move:filepath",
                                  "-P", download_dir, url],
                                 capture_output=True, check=True)
            #strip newline
            file = res.stdout.decode(*ENCODE_ARGS).strip().splitlines()[-1]
        except subprocess.CalledProcessError as e:
...
def requeue_workflow_unchecked():
    ...
    prompt = prompt.copy()
    for uid in prompt:
        if prompt[uid]['class_type'] == 'KKvideo_BatchManager':
...
def requeue_workflow(requeue_required=(-1,True)):
    ...
    if requeue_guard[0] != run_number:
        ...
        for bm_uid in prompt:
            if prompt[bm_uid]['class_type'] == 'KKvideo_BatchManager':
                for output_uid in prompt:
                    if prompt[output_uid]['class_type'] in ["KKvideo_VideoCombine"]:
...
def hash_path(path):
    if path is None:
        return "input"
    if is_url(path):
        return hashlib.sha256(strip_path(path).encode()).hexdigest()
