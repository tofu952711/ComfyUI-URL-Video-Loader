...
const convDict = {
    KKvideo_LoadImages : ["directory", null, "image_load_cap", "skip_first_images", "select_every_nth"],
    KKvideo_LoadImagesPath : ["directory", "image_load_cap", "skip_first_images", "select_every_nth"],
    KKvideo_VideoCombine : ["frame_rate", "loop_count", "filename_prefix", "format", "pingpong", "save_image"],
    KKvideo_LoadVideo : ["video", "force_rate", "force_size", "frame_load_cap", "skip_first_frames", "select_every_nth"],
    KKvideo_LoadVideoPath : ["video", "force_rate", "force_size", "frame_load_cap", "skip_first_frames", "select_every_nth"],
};
const renameDict  = {KKvideo_VideoCombine : {save_output : "save_image"}}
    ...
    chainCallback(nodeType.prototype, "onNodeCreated", function() {
        ...
        previewWidget.updateSource = function () {
            ...
            if (!advp) {
                ...
                element.src = api.apiURL('/kkvideo/viewaudio?' + new URLSearchParams(params));
            }
        }
        ...
        previewWidget.updateSource = function () {
            ...
            if (params.format?.split('/')[0] == 'video'
                ...
                if (!advp) {
                    ...
                    params.deadline = app.ui.settings.getSettingValue("VHS.AdvancedPreviewsDeadline")
                    this.videoEl.src = api.apiURL('/kkvideo/viewvideo?' + new URLSearchParams(params));
                }
            ...
            const doQuery = async () => {
                ...
                let qurl = api.apiURL('/kkvideo/queryvideo?' + new URLSearchParams(previewWidget.value.params))
                let query = undefined
                try {
...
function searchBox(event, [x,y], node) {
    ...
    async function updateOptions() {
        ...
        if (last_path != path) {
            ...
            let optionsURL = api.apiURL('/kkvideo/getpath?' + new URLSearchParams(params));
            try {
...
app.registerExtension({
    ...
    async beforeRegisterNodeDef(nodeType, nodeData, app) {
        if(nodeData?.name?.startsWith("KKvideo_")) {
            useKVState(nodeType);
        ...
        if (nodeData?.name == "KKvideo_LoadImages") {
        ...
        } else if (nodeData?.name == "KKvideo_LoadImagesPath") {
            chainCallback(nodeType.prototype, "onNodeCreated", function() {
                const pathWidget = this.widgets.find((w) => w.name === "directory");
                chainCallback(pathWidget, "callback", (value) => {
                    if (!value) {
            ...
            addLoadCommon(nodeType, nodeData);
        } else if (nodeData?.name == "KKvideo_LoadVideo" || nodeData?.name == "KKvideo_LoadVideoFFmpeg") {
            chainCallback(nodeType.prototype, "onNodeCreated", function() {
                const pathWidget = this.widgets.find((w) => w.name === "video_url" || w.name === "video");
                chainCallback(pathWidget, "callback", (value) => {
                    if (!value) {
                        return;
                    }
                    ...
                    let type = isUrl ? "path" : "input";
                    let extension_index = value.lastIndexOf(".");
                    let extension = value.slice(extension_index+1).split(/[?#]/)[0].toLowerCase();
                    let format = "video"
                    if (["gif", "webp", "avif"].includes(extension)) {
                        format = "image"
                    }
                    format += "/" + extension;
                    let params = {filename : value, type, format: format};
                    this.updateParameters(params, true);
                });
            });
            addLoadCommon(nodeType, nodeData);
            addVAEOutputToggle(nodeType, nodeData);
        } else if (nodeData?.name == "KKvideo_LoadAudio") {
        ...
        } else if (nodeData?.name == "KKvideo_LoadAudioUpload") {
        ...
        } else if (nodeData?.name == "KKvideo_LoadVideoPath" || nodeData?.name == "KKvideo_LoadVideoFFmpegPath") {
            chainCallback(nodeType.prototype, "onNodeCreated", function() {
            ...
            addLoadCommon(nodeType, nodeData);
            addVAEOutputToggle(nodeType, nodeData);
        } else if (nodeData?.name == "KKvideo_LoadImagePath") {
            addLoadCommon(nodeType, nodeData);
            addVAEOutputToggle(nodeType, nodeData);
            chainCallback(nodeType.prototype, "onNodeCreated", function() {
        ...
        } else if (nodeData?.name == "KKvideo_VideoCombine") {
    ...
    async loadedGraphNode(node) {
        ...
        if (node.type?.startsWith("KKvideo_") && node.inputs) {
    ...
    async setup() {
        ...
        document.addEventListener('paste', async (e) => {
            ...
            if (filepath && copiedPath == filepath) {
                ...
                const pastedNode = LiteGraph.createNode('KKvideo_LoadVideoFFmpeg')
    ...
    async init() {
        ...
        if (app.KKvideoHelp != helpDOM) {
            helpDOM = app.KKvideoHelp
        } else {
            initHelpDOM()
        }
    ...
    if (detail === null) {
        for (let graph of [app.graph, ...app.graph.subgraphs.values()]) {
            for (let node of graph._nodes) {
                if (node.type.startsWith("KKvideo_")) {
...
api.addEventListener('KKvideo_latentpreview', ({ detail }) => {
    if (detail.id == null) {
        return
    }
